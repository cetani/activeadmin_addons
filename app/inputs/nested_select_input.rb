class NestedSelectInput < ActiveAdminAddons::InputBase
  ...
  def set_parent_attributes(hierarchy)
    hierarchy.each_with_index do |level_data, idx|
      ...
      if parent_level_data
        ...
        level_data[:parent_is_field] = parent_level_data[:is_field]
  ...
  def set_parent_value(level_data)
    ...
    build_virtual_attr(parent_attribute, level_data[:parent_is_field])
  ...
  def build_virtual_attr(attribute_name, is_field)
    if @object.respond_to?(attribute_name)
      return if is_field
      fail "#{attribute_name} is already defined"
    else
      fail "#{attribute_name} can't be optional and not defined" if is_field
